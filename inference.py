...
from finetune.models.src.motion_stage1 import (
    MOTION_STATE_PREFIX,
    Stage1MotionPipeline,
    build_motion_config_from_args,
    default_motion_config,
    is_motion_enabled,
    load_motion_pipeline_weights,
)
...
def _has_embedded_camo_motion(model_path):
    """Return whether the transformer checkpoint embeds CAMO motion weights."""
    ...
    for index_name in index_names:
        ...
        if index_path.exists():
            with open(index_path, "r", encoding="utf-8") as f:
                checkpoint_keys = list(json.load(f).get("weight_map", {}))
            break

    return any(key.startswith("camo_motion.") for key in checkpoint_keys)


def _load_pipeline_with_embedded_motion(model_path, dtype, motion_config):
    """Load sharded checkpoints after registering their custom motion module."""
    if not _has_embedded_camo_motion(model_path):
        pipe = CogVideoXPipeline.from_pretrained(model_path, torch_dtype=dtype)
        return pipe, None

    original_init = CogVideoXTransformer3DModel.__init__
    ...
    def init_with_motion(self, *args, **kwargs):
        ...
        motion_pipeline = Stage1MotionPipeline(
            ...
            latent_channels=int(self.config.in_channels),
        )
        setattr(self, "camo_motion", motion_pipeline)
        motion_pipeline.attach_dit_reliability_injector(self)

    CogVideoXTransformer3DModel.__init__ = init_with_motion
    try:
        pipe = CogVideoXPipeline.from_pretrained(model_path, torch_dtype=dtype)
    finally:
        CogVideoXTransformer3DModel.__init__ = original_init

    motion_pipeline = pipe.transformer.camo_motion
    motion_pipeline.eval()
    print(
        f"Loaded embedded CAMO motion weights from "
        f"{Path(model_path) / 'transformer'} (camo_motion.*)"
    )
    return pipe, motion_pipeline
...
def count_module_parameters(module, seen_param_ids=None):
    """Count parameters in one module, optionally skipping params already seen."""
...
def count_model_parameters_by_component(pipe, motion_pipeline=None):
    """Count unique parameters for each pipeline component and optional motion pipeline."""
    seen_param_ids = set()
    component_stats = []
    ...
    if hasattr(pipe, "components"):
        for name, module in pipe.components.items():
            if not hasattr(module, "parameters"):
                continue
            total_params, trainable_params = count_module_parameters(module, seen_param_ids)
            component_stats.append((name, total_params, trainable_params))
    elif hasattr(pipe, "parameters"):
        total_params, trainable_params = count_module_parameters(pipe, seen_param_ids)
        component_stats.append(("pipeline", total_params, trainable_params))

    if motion_pipeline is not None and hasattr(motion_pipeline, "parameters"):
        total_params, trainable_params = count_module_parameters(motion_pipeline, seen_param_ids)
        component_stats.append(("motion_pipeline", total_params, trainable_params))

    total_params = sum(stats[1] for stats in component_stats)
    trainable_params = sum(stats[2] for stats in component_stats)
    return total_params, trainable_params, component_stats


def print_model_parameter_summary(pipe, motion_pipeline=None):
    total_params, trainable_params, component_stats = count_model_parameters_by_component(
        pipe, motion_pipeline
    )
    print(
        "Model parameters: "
        f"{format_param_count(total_params)} ({total_params:,}) total | "
        f"{format_param_count(trainable_params)} ({trainable_params:,}) trainable"
    )
    print("Model parameters by component:")
    for name, component_total, component_trainable in component_stats:
        print(
            f"  - {name}: "
            f"{format_param_count(component_total)} ({component_total:,}) total | "
            f"{format_param_count(component_trainable)} ({component_trainable:,}) trainable"
        )
...
def process_video(
    ...
    motion_pipeline: Optional[Stage1MotionPipeline] = None,
):
    ...
    video = video.to(execution_device, dtype=pipe.vae.dtype)
    ...
    if (
        ...
        rho_t = motion_pipeline.estimate_reliability(m_obj_lr, m_turb_lr)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Video super-resolution with CAMO")
    motion_defaults = default_motion_config()["motion_pipeline"]
    ...
    parser.add_argument(
        "--motion_weight_prefix",
        type=str,
        default=MOTION_STATE_PREFIX,
        help="Checkpoint namespace used for CAMO motion-module weights.",
    )
    ...
    parser.add_argument(
        "--is_cpu_offload",
        action="store_true",
        help="Enable component-level CPU offload for lower GPU memory use",
    )
    ...
    parser.add_argument("--save_format", type=str, default="yuv444p", help="FFmpeg output pixel format")
    ...
    parser.add_argument(
        "--enable_motion_decoupling",
        action=argparse.BooleanOptionalAction,
        default=motion_defaults["enable_decoupling"],
    )
    parser.add_argument(
        "--enable_motion_turbulence_rectifier",
        action=argparse.BooleanOptionalAction,
        default=motion_defaults["enable_turbulence_rectifier"],
    )
    parser.add_argument(
        "--enable_motion_object_enhancement",
        action=argparse.BooleanOptionalAction,
        default=motion_defaults["enable_object_motion_enhancement"],
    )
    parser.add_argument(
        "--enable_motion_reliability_injection",
        action=argparse.BooleanOptionalAction,
        default=motion_defaults["enable_reliability_injection"],
    )
    parser.add_argument(
        "--enable_motion_reliability_velocity_scaling",
        action=argparse.BooleanOptionalAction,
        default=motion_defaults["enable_reliability_velocity_scaling"],
    )
    ...
    parser.add_argument("--motion_adapter_align_corners", action="store_true")
    ...
    prompt_embedding_name = "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855.safetensors"
    checkpoint_prompt_path = Path(args.model_path) / "prompt_embeddings" / prompt_embedding_name
    repository_prompt_path = Path(__file__).resolve().parent / "pretrained_models" / "prompt_embeddings" / prompt_embedding_name
    empty_prompt_path = checkpoint_prompt_path if checkpoint_prompt_path.exists() else repository_prompt_path
    ...
    motion_config = build_motion_config_from_args(args)
    pipe, motion_pipeline = _load_pipeline_with_embedded_motion(
        args.model_path, dtype, motion_config
    )
    ...
    if is_motion_enabled(motion_config) and motion_pipeline is None:
        latent_channels = int(pipe.transformer.config.in_channels)
        motion_pipeline = Stage1MotionPipeline(
            config=motion_config,
            latent_channels=latent_channels,
        )
        setattr(pipe.transformer, "camo_motion", motion_pipeline)
        motion_pipeline.attach_dit_reliability_injector(pipe.transformer)
        motion_pipeline.eval()

        transformer_dir = Path(args.model_path) / "transformer"
        loaded = load_motion_pipeline_weights(
            motion_pipeline,
            transformer_dir,
            state_prefix=args.motion_weight_prefix,
        )
        if loaded:
            print(f"Loaded motion weights from {transformer_dir}")
        else:
            raise FileNotFoundError(
                "Motion is enabled, but the checkpoint does not contain the required "
                f"motion weights under {transformer_dir}. Use the complete released "
                "CAMO checkpoint."
            )
    ...
    if args.is_cpu_offload:
        # Component-level offload is compatible with CogVideoX VAE tiling and
        # keeps only the active pipeline component on the GPU.
    ...
    print_model_parameter_summary(pipe, motion_pipeline)
    ...
    if args.eval_metrics != '':
        ...
        for name in metrics_list:
            try:
                execution_device = getattr(pipe, "_execution_device", pipe.device)
                metrics_models[name] = pyiqa.create_metric(name).to(execution_device).eval()
            except Exception as e:
    ...
    for video_path in tqdm(video_files, desc="Processing videos"):
        ...
        if os.path.exists(video_path):
            ...
            print(
                f"Inference time: {video_name} = {inference_elapsed:.3f}s | "
                f"Average per video = {avg_inference_time:.3f}s ({len(inference_times)} videos)"
            )
    ...
    if inference_times:
        ...
        print(f"Processed videos: {len(inference_times)}")
        print(f"Average inference time per video: {avg_inference_time:.3f}s")
