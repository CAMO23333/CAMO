...
from finetune.mydatasets.degradation import RandomBlur, RandomResize, RandomNoise, RandomJPEGCompression, RandomVideoCompression, DegradationsWithShuffle
from finetune.mydatasets.utils import read_video_frames, random_crop_frames, paired_random_crop_video
...
class RealSRDataset(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.lq_videos = (
            load_videos_with_root(lq_video_column, data_root)
            if lq_video_column is not None
            else None
        )
        self.use_paired_video = self.lq_videos is not None
        if self.use_paired_video:
            gt_ids = [path.stem for path in self.videos]
            lq_ids = [path.stem for path in self.lq_videos]
            if gt_ids != lq_ids:
                raise ValueError(
                    "GT and LQ directories must contain the same video IDs in sorted order. "
                    f"First mismatch: GT={gt_ids[:3]} vs LQ={lq_ids[:3]}"
                )
        ...
        self.enable_random_frame_drop = trainer.args.enable_random_frame_drop
        self.max_frame_drop_count = trainer.args.max_frame_drop_count
        empty_prompt_path = (
            self.trainer.args.data_root
            / "cache"
            / self.prompt_cache
            / "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855.safetensors"
        )
        self.empty_prompt = None
        if trainer.args.empty_prompt:
            if empty_prompt_path.exists():
                self.empty_prompt = load_file(empty_prompt_path)["prompt_embedding"]
            else:
                # Compute once in the main process before DataLoader workers start.
                # Every distributed rank keeps an in-memory copy; only rank 0 writes it.
                self.empty_prompt = self.encode_text("")[0].detach().cpu()
                if self.is_cache and trainer.accelerator.is_main_process:
                    empty_prompt_path.parent.mkdir(parents=True, exist_ok=True)
                    save_file(
        ...
        if self.use_paired_video and any(not path.is_file() for path in self.lq_videos):
            raise ValueError(
                "Some LQ video files were not found. "
                f"Missing file: {next(path for path in self.lq_videos if not path.is_file())}"
            )
        ...
        if self.use_paired_video and len(self.videos) != len(self.lq_videos):
            raise ValueError(
                "Expected HR/LQ video list length to be the same but found "
                f"{len(self.videos)=} and {len(self.lq_videos)=}."
            )

        if self.use_paired_video:
            logger.info("RealSRDataset is using paired HR/LQ video lists for training.")

    def _random_drop_frames(self, frame_list: List[np.ndarray]) -> List[np.ndarray]:
        if (
            not self.enable_random_frame_drop
            or self.max_frame_drop_count <= 0
            or len(frame_list) <= 1
        ):
            return frame_list

        max_drop = min(self.max_frame_drop_count, len(frame_list) - 1)
        drop_count = random.randint(0, max_drop)
        if drop_count == 0:
            return frame_list

        drop_indices = set(random.sample(range(len(frame_list)), drop_count))
        return [f for i, f in enumerate(frame_list) if i not in drop_indices]

    @staticmethod
    def _pad_to_num_frames(frame_list: List[np.ndarray], target_frames: int) -> List[np.ndarray]:
        if len(frame_list) >= target_frames:
            return frame_list
        return frame_list + [frame_list[-1]] * (target_frames - len(frame_list))

    def _random_drop_paired_frames(
        self,
        hq_frame_list: List[np.ndarray],
        lq_frame_list: List[np.ndarray],
    ) -> Tuple[List[np.ndarray], List[np.ndarray]]:
        if (
            not self.enable_random_frame_drop
            or self.max_frame_drop_count <= 0
            or len(hq_frame_list) <= 1
        ):
            return hq_frame_list, lq_frame_list

        max_drop = min(self.max_frame_drop_count, len(hq_frame_list) - 1)
        drop_count = random.randint(0, max_drop)
        if drop_count == 0:
            return hq_frame_list, lq_frame_list

        drop_indices = set(random.sample(range(len(hq_frame_list)), drop_count))
        hq_kept = [f for i, f in enumerate(hq_frame_list) if i not in drop_indices]
        lq_kept = [f for i, f in enumerate(lq_frame_list) if i not in drop_indices]
        return hq_kept, lq_kept
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        is_empty_prompt = random.random() < self.trainer.args.empty_ratio
        if is_empty_prompt:
            prompt = ''

        video = self.videos[index]
        lq_video = self.lq_videos[index] if self.use_paired_video else None
        train_resolution_str = "x".join(str(x) for x in self.trainer.args.train_resolution)

        # Current shape of frames: [F, C, H, W]
        hq_frames, lq_frames = self.preprocess(video, lq_video)
    ...
    def preprocess(self, video_path: Path, lq_video_path: Path | None = None) -> torch.Tensor:
        """
        ...
        if self.crop_mode == 'random_crop':
            if lq_video_path is not None:
                hq_frame_list = read_video_frames(video_path, self.max_num_frames)
                lq_frame_list = read_video_frames(lq_video_path, self.max_num_frames)

                # Align temporal length for paired crop.
                min_frames = min(len(hq_frame_list), len(lq_frame_list))
                hq_frame_list = hq_frame_list[:min_frames]
                lq_frame_list = lq_frame_list[:min_frames]

                hq_frame_list, lq_frame_list = self._random_drop_paired_frames(
                    hq_frame_list, lq_frame_list
                )

                if len(hq_frame_list) < self.max_num_frames:
                    hq_frame_list = self._pad_to_num_frames(hq_frame_list, self.max_num_frames)
                    lq_frame_list = self._pad_to_num_frames(lq_frame_list, self.max_num_frames)
            ...
            frame_list = self._random_drop_frames(frame_list)
            frame_list = self._pad_to_num_frames(frame_list, self.inter_frames)
