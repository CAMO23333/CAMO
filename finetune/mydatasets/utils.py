...
def load_videos_with_root(video_path: Path, root_path: Path) -> List[Path]:
    """Load videos from a released directory, with legacy list-file support.

    The public CAMO dataset release contains paired ``gt/`` and ``turb_lr/``
    directories, so training no longer requires users to create ``.txt``
    manifests.  Keeping list-file support here makes older private datasets
    readable without making it part of the documented workflow.
    """
    source = Path(video_path)
    if not source.exists() and not source.is_absolute():
        rooted_source = Path(root_path) / source
        if rooted_source.exists():
            source = rooted_source
    ...
    if source.is_dir():
        ...
        if not videos:
            raise ValueError(f"No supported video files found in {source}")
        return videos
    ...
    raise FileNotFoundError(f"Video directory or list file does not exist: {source}")
