...
from finetune.mydatasets import RealSRDataset
from finetune.mydatasets.utils import (
    load_images,
    load_prompts,
    load_videos,
...
class Trainer:
    ...
    def _init_distributed(self):
        ...
        if os.environ.get("DEBUG_RANK0", "0") == "1":
            import debugpy

            if accelerator.is_main_process:
                debugpy.listen(("0.0.0.0", 5678))
                print("Waiting for VSCode debugger attach on rank0...")
    ...
    @staticmethod
    def _sanitize_tracker_value(value: Any) -> Any:
    ...
    def _sanitize_tracker_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        return {k: self._sanitize_tracker_value(v) for k, v in config.items()}

    def prepare_trackers(self) -> None:
        logger.info("Initializing trackers")
        exp_name = self.args.output_dir.name
        tracker_name = self.args.tracker_name or "finetrainers-experiment"
        tracker_config = self._sanitize_tracker_config(self.args.model_dump())
        self.accelerator.init_trackers(tracker_name, config=tracker_config, init_kwargs={
            "wandb": {
                "name": exp_name,
            }
        })
    ...
    def __maybe_save_checkpoint(self, global_step: int, must_save: bool = False):
        if (
            ...
            if must_save or global_step % self.args.checkpointing_steps == 0:
                ...
                self.on_checkpoint_saved(Path(save_path))
    ...
    def on_checkpoint_saved(self, checkpoint_path: Path) -> None:
        """Hook for subclasses to persist extra files into each checkpoint directory."""
        return

    def clear_runtime_contexts(self) -> None:
        """Hook for subclasses to clear temporary forward contexts after backward."""
        return
