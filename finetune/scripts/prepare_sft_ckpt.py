...
def run_zero_to_fp32(checkpoint_dir, output_dir):
    ...
    converter = Path(__file__).resolve().with_name("zero_to_fp32.py")
    subprocess.run([
...
def main():
    ...
    parser.add_argument("--weights_source", default="pretrained_models/CogVideoX1.5-5B",
    ...
    if args.ckpt_output_dir == "":
        ...
        ckpt_output_dir = str(checkpoint_path.parent / f"{checkpoint_path.name}-sft")
    else:
